        ...
        try {
            ...
            if (dX > 0) {
                ...
                if (swipeLeftBackgroundColor != 0) {
                    ...
                    background.setBounds(
                        viewHolder.itemView.left, viewHolder.itemView.top, viewHolder.itemView.right, viewHolder.itemView.bottom
                    )
                    background.draw(canvas)
                }
                var iconSize = 0
                ...
                if (mSwipeLeftText != null && mSwipeLeftText!!.length > 0 && dX < -iconHorizontalMargin - iconSize) {
                    ...
                    val widthText = textPaint.measureText(mSwipeLeftText)
                    val textTop =
                        (viewHolder.itemView.top + (viewHolder.itemView.bottom - viewHolder.itemView.top) / 2.0 + textPaint.textSize / 2).toInt()
                    canvas.drawText(
                        mSwipeLeftText!!,
                        imgLeft - widthText - if (imgLeft == viewHolder.itemView.right) iconHorizontalMargin else iconHorizontalMargin / 2,
                        textTop.toFloat(),
                        textPaint
                    )
                }
